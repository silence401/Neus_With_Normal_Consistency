...
class NeuSRenderer:
    ...
    def render_core(self,
        ...
        #print(rays_d.shape)
        jitter_std = self.jitter_network(torch.Tensor(0))
        rays_o_jitter = rays_o + torch.normal(mean=0, std=jitter_std, size=rays_o.shape, device=rays_o.device)
        ...
        pts_jitter = pts_jitter.reshape(-1, 3)
        pts = pts.reshape(-1, 3)
        dirs = dirs.reshape(-1, 3)
        ...
        iter_cos_jitter = -(F.relu(-true_cos_jitter * 0.5 + 0.5) * (1.0 - cos_anneal_ratio) + 
                            F.relu(-true_cos_jitter) * cos_anneal_ratio)

        # Estimate signed distances at section points
        estimated_next_sdf = sdf + iter_cos * dists.reshape(-1, 1) * 0.5
        estimated_prev_sdf = sdf - iter_cos * dists.reshape(-1, 1) * 0.5

        estimated_next_sdf_jitter = sdf_jitter + iter_cos_jitter * dists.reshape(-1, 1) * 0.5 
        estimated_prev_sdf_jitter = sdf_jitter - iter_cos_jitter * dists.reshape(-1, 1) * 0.5
